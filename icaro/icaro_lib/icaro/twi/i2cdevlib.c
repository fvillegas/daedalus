...
int8_t i2c_read_bit(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitNum, uint8_t *data, uint16_t timeout)
{
    uint8_t b;
...
int8_t i2c_read_bit_word(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitNum, uint16_t *data, uint16_t timeout)
{
    uint16_t b;
...
int8_t i2c_read_bits(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitStart, uint8_t length, uint8_t *data, uint16_t timeout) {
    // 01101001 read byte
...
int8_t i2c_read_bits_word(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitStart, uint8_t length, uint16_t *data, uint16_t timeout) {
...
int8_t i2c_read_bytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint8_t *data, uint16_t timeout)
{
    ...
    for (uint8_t k = 0; k < length; k += min((int)length, TWI_BUFFER_LENGTH)) {
        
        // Wire.beginTransmission(dev_addr);
        // Wire.write(reg_addr);
        // Wire.endTransmission();
        
        twi_write_to(dev_addr, [reg_addr], TRUE, TRUE, 0);
        
        twi_read_from(dev_addr, data, length, TRUE);
        
        // Wire.beginTransmission(dev_addr);
        // Wire.requestFrom(dev_addr, (uint8_t)min(length - k, TWI_BUFFER_LENGTH));
...
int8_t I2Cdev::readWords(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint16_t *data, uint16_t timeout) {
    ...
    uint32_t t1 = millis();
            ...
            for (uint8_t k = 0; k < length * 2; k += min(length * 2, BUFFER_LENGTH)) {
                ...
                Wire.endTransmission();
                Wire.beginTransmission(dev_addr);
                Wire.requestFrom(dev_addr, (uint8_t)(length * 2)); // length=words, this wants bytes
        ...
        uint8_t status = Fastwire::readBuf(dev_addr << 1, reg_addr, intermediate, (uint8_t)(length * 2));
        if (status == 0) {
 ...
 */
bool I2Cdev::writeBit(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitNum, uint8_t data) {
    uint8_t b;
 ...
 * @return Status of operation (true = success)
 */
bool I2Cdev::writeBitW(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitNum, uint16_t data) {
    uint16_t w;
 ...
 * @return Status of operation (true = success)
 */
bool I2Cdev::writeBits(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitStart, uint8_t length, uint8_t data) {
    //      010 value to write
    // 76543210 bit numbers
    //    xxx   args: bitStart=4, length=3
    // 00011100 mask byte
 ...
 * @return Status of operation (true = success)
 */
bool I2Cdev::writeBitsW(uint8_t dev_addr, uint8_t reg_addr, uint8_t bitStart, uint8_t length, uint16_t data) {
    //              010 value to write
    // fedcba9876543210 bit numbers
    //    xxx           args: bitStart=12, length=3
...
bool I2Cdev::writeBytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint8_t* data) {
    ...
    uint8_t status = 0;
        ...
        Wire.beginTransmission(dev_addr);
        Wire.send((uint8_t) reg_addr); // send address
    #elif ((I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE && ARDUINO >= 100) \
            || (I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_SBWIRE && ARDUINO >= 100) \
            || I2CDEV_IMPLEMENTATION == I2CDEV_TEENSY_3X_WIRE)
        Wire.beginTransmission(dev_addr);
        Wire.write((uint8_t) reg_addr); // send address
    #elif (I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_FASTWIRE)
        Fastwire::beginTransmission(dev_addr);
        Fastwire::write(reg_addr);
    #endif
    for (uint8_t i = 0; i < length; i++) {
        #ifdef I2CDEV_SERIAL_DEBUG
...
bool I2Cdev::writeWords(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint16_t* data) {
    ...
    #if ((I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE && ARDUINO < 100) || I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_NBWIRE)
        Wire.beginTransmission(dev_addr);
        Wire.send(reg_addr); // send address
    #elif ((I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE && ARDUINO >= 100) \
            || (I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_SBWIRE && ARDUINO >= 100) \
            || I2CDEV_IMPLEMENTATION == I2CDEV_TEENSY_3X_WIRE)
        Wire.beginTransmission(dev_addr);
