...
void set_mode(uint8_t new_mode)
{
	...
	// requirement specified in the datasheet; it's actually more efficient than
	// using the I2Cdev.writeBits method
	i2c_write_byte(HMC5883L_ADDRESS, HMC5883L_MODE, new_mode << (HMC5883L_MODEREG_BIT - HMC5883L_MODEREG_LENGTH + 1));
	mode = new_mode; // track to tell if we have to clear bit 7 after a read
}
...
void hcm5883l_init()
{
	...
	set_gain(HMC5883L_GAIN_1090);

	// write MODE register
	set_mode(HMC5883L_MODE_SINGLE);
}
